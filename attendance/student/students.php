...
ob_start();
session_start();
//vérifie si la variable de session 'name' n'est pas égale à 'oasis'. Cela suppose que 'oasis' est une valeur spécifique stockée dans la session pour vérifier l'authentification de l'utilisateur.
//Si la condition n'est pas satisfaite (c'est-à-dire si l'utilisateur n'est pas authentifié), le script redirige l'utilisateur vers la page de connexion (login.php) en utilisant header('location: login.php');.
        ...
        <div class="content">
            ...
            <form method="post" action="">
                <table class="table table-striped">
                    ...
                    <tbody>
                    <tr>
                        <!-- //Cette cellule affiche l'identifiant de l'étudiant (st_id). -->
                        ...
                        <!-- Cette cellule affiche le statut de présence de l'étudiant (attendance_status). Si le statut est "présent", la classe CSS "present" est appliquée à la cellule, sinon la classe "absent" est appliquée. -->
            ...
            <?php
            // Il vérifie d'abord si le formulaire a été soumis en vérifiant si la clé mark_attendance est présente dans la variable $_POST. Cela signifie que l'utilisateur a cliqué sur le bouton de soumission du formulaire.
         ...
         header("Refresh:0");
        // a page est actualisée à l'aide de la fonction header("Refresh:0"). Cette fonction spécifie un délai de rafraîchissement de 0 seconde, ce qui signifie que la page est actualisée immédiatement après l'affichage du message de confirmation. Cela permet à l'utilisateur de voir instantanément les mises à jour apportées à l'affichage de la présence 
